...
### FUNZIONA ####
"""
Nuova versione modificata che prova ad intergrare snowboy

Sottoscrive le richieste umane in forma di testo ,
 le invia a dialogflow per le elaborazioni
 risponde a voce
"""
...
sys.path.append(os.path.join(os.path.dirname(__file__), '../../scripts/snowboy'))
...
sys.path.append('/home/pi/ros/src/dialogflow_ros/src/dialogflow_ros/utils')#per importare utils
...
class DialogflowClient(object):
    def __init__(self, language_code='it-IT', last_contexts=None):
        ...
        with open(file_dir, 'r') as f:
            try:
                self.phrase_hints = load(f) ###self.phrase_hints = load(f)
            except YAMLError:
        ...
        #snowboy
        self.model_path = '/home/pi/snowboy/OK_Robot.pmdl'
        rospy.loginfo("snowboy model:"+self.model_path)
    ...
    def exit(self):
        ...
        self.detector.terminate() #snowboy terminate
