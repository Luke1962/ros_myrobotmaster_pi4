...
/// ///////////////////////////////////////////////////////////////////
	...
	struct System_t
	{
		...
		bool pid_enabled;
		float delta_encoders_target; 
		float pidL_encoder_setpoint;
		float pidR_encoder_setpoint;
		float pid_target_tolerance;
	};
...
/// ////////////////////////////////////////////////////////////////////////////////////////////	
/// ////////////////////////////////////////////////////////////////////////////////////////////
 	std_msgs::Float64	msg_pidL_setpoint; 
 	std_msgs::Float64	msg_pidR_setpoint;
	ros::Publisher pub_pidL_setpoint;
	ros::Publisher pub_pidR_setpoint;
	
	ros::Publisher pub_pidL_enable;
	ros::Publisher pub_pidR_enable;
	
 	//std_msgs::Float	msg_pidL_state; 
 	//std_msgs::Float	msg_pidR_state;

	std_msgs::Bool		msg_pidL_enable; //abilitato dal cbk_targetDeltaPose , disabilitato dal raggiungimento
	std_msgs::Bool		msg_pidR_enable;
...
/// ////////////////////////////////////////////////////////////////////////////////////////////
	void init_globalVars()
	{
		...
		thisSystem.pid_enabled = false;
		thisSystem.delta_encoders_target=0;
...
/// ///////////////////////////////////////////////////////////////
/// ////////////////////////////////////////////////////////////////
	void cbk_targetDeltaPose(const geometry_msgs::Twist &msg){
		ROS_INFO( "\e[095m *** Ricevuto target pose Lin: %f, Ang: %f \e[0m ", msg.linear.x , msg.angular.z);
		
		//salvo nella variabile globale, convertendo i gradi richiesti in radianti
		msg_spostamentoRichiesto.linear.x  =  msg.linear.x ;
		msg_spostamentoRichiesto.angular.z =  (float)msg.angular.z * M_PI/180;
		
		
		if (msg.linear.x !=0) //spostamento
		{
			// Calcolo gli spostamenti di ciascuna ruota rispetto al terreno in termini di encoder ticks		
			thisSystem.delta_encoders_target  = (msg.linear.x *1000 /  thisSystem.par.enc2mm);  // enc2mm=0.0645
			
			thisSystem.pidL_encoder_setpoint =  (float)thisSystem.encL.count +  thisSystem.delta_encoders_target;
			thisSystem.pidR_encoder_setpoint =  (float)thisSystem.encR.count +  thisSystem.delta_encoders_target;
			
			
			// assegno il setpoint ai PID
			msg_pidL_setpoint.data = thisSystem.pidL_encoder_setpoint ;
			msg_pidR_setpoint.data = thisSystem.pidR_encoder_setpoint ;
			pub_pidL_setpoint.publish(msg_pidL_setpoint);
			pub_pidR_setpoint.publish(msg_pidR_setpoint);				

			// abilito i PID
			thisSystem.pid_enabled = true;
			msg_pidL_enable.data = thisSystem.pid_enabled;			
			msg_pidR_enable.data = thisSystem.pid_enabled;
			pub_pidL_enable.publish(msg_pidL_enable);					
			pub_pidR_enable.publish(msg_pidR_enable);					
			ROS_INFO( "\e[095m ENCODER LEFT  Target(pid setpoint)  %f, current(pid state)  %d, delta (error): %f \e[0m", msg_pidL_setpoint.data , thisSystem.encL.count, thisSystem.delta_encoders_target );

		}else if (msg.angular.z !=0 ) // rotazione
		{
			// delta_mm = radianti * raggio
			float delta_mm = 0.5* (msg.angular.z * M_PI/180 ) * ((double)thisSystem.par.wheelDistance * 1000);
			// converto nei valori degli encoders
			thisSystem.delta_encoders_target  = delta_mm / thisSystem.par.enc2mm ;
			thisSystem.pidL_encoder_setpoint =  (float)thisSystem.encL.count -  thisSystem.delta_encoders_target;
			thisSystem.pidR_encoder_setpoint =  (float)thisSystem.encR.count +  thisSystem.delta_encoders_target;
			...
			pub_pidL_enable.publish(msg_pidL_enable);					
			pub_pidR_enable.publish(msg_pidR_enable);				
			ROS_INFO( "ENCODER LEFT  Target(setpoint)  %f, current(state)  %d, delta (error): %f", msg_pidL_setpoint.data , thisSystem.encL.count, thisSystem.delta_encoders_target );
...
// monitora l'errore rispetto al target.
// quando viene raggiunta la posizione entro l'errore ammesso 
// disabilita i PID
void pidControlLoop(float target_tolerance){
	if (thisSystem.pid_enabled )
	{
		float error_mm = thisSystem.par.enc2mm * ( fabs(thisSystem.encL.count - thisSystem.pidL_encoder_setpoint) + fabs( thisSystem.encR.count - thisSystem.pidR_encoder_setpoint));
		if (error_mm < (1000 * target_tolerance) ) //target considerato raggiunto
		{
			...
			// mi fermo (per sicurezza. dovrebbe essere superfluo
			msg_cmdVel = msg_fill_cmdVel( 0.0 ,0.0);
			pub_cmdVel.publish(msg_cmdVel);
			msg_spostamentoRichiesto.linear.x = 0.0;
			
			// segnalo
			ROS_INFO("\t\t \e[032m **POSIZIONE RAGGIUNTA** \e[0m");  
			//speech("fatto");
		}else
		{
			ROS_INFO_THROTTLE(1,"\e[095m Enc. Target Error mm : %f  > tolreance: %f \e[0m", error_mm, target_tolerance);
		}
...
/// ////////////////////////////////////////////////////////////////////////////////////////////
	int main(int argc, char **argv)
	{
		...
		nh.param<float>("target_tolerance", thisSystem.pid_target_tolerance, 0.02);
		...
		ros::Subscriber sub_targetPose = nh.subscribe("/target_pose", 1, cbk_targetDeltaPose);
		...
		pub_pidL_setpoint = nh.advertise<std_msgs::Float64>("/left_wheel/setpoint", 1); 
		pub_pidR_setpoint = nh.advertise<std_msgs::Float64>("/right_wheel/setpoint", 1); 
		
		//pub_pidL_state = nh.advertise<std_msgs::Float64>("/left_wheel/state", 1); 
		//pub_pidR_state = nh.advertise<std_msgs::Float64>("/right_wheel/state", 1); 

		pub_pidL_enable = nh.advertise<std_msgs::Bool>("/left_wheel/pid_enable", 1); 
		pub_pidR_enable = nh.advertise<std_msgs::Bool>("/right_wheel/pid_enable", 1); 
		//------------------------------------------------------------------------------------------------
		...
		while (nh.ok())
		{
			...
			pidControlLoop( thisSystem.pid_target_tolerance );
