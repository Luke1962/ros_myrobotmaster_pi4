...
void Adafruit_INA219::wireWriteRegister(uint8_t reg, uint16_t value) {
  _i2c->address(ina219_i2caddr); 
   _i2c->writeWordReg(reg, value);	
...
void Adafruit_INA219::wireReadRegister(uint8_t reg, uint16_t *value) {
	try
	{
	  ...
	  _i2c->writeByte(reg);
	  _i2c->address(ina219_i2caddr);
	  _i2c->read(rx_tx_buf, DATA_REG_SIZE);
	 
	  *value = ( (rx_tx_buf[0]) << 8) | rx_tx_buf[1];			
	}
	catch (...)
	{
		printf("\n*** Exception reading register in Adafruit_INA219::wireReadRegister at reg: %d   ", reg);
	}
 ...
 */
void Adafruit_INA219::begin(mraa::I2c* i2c) {
  _i2c = i2c;
  init();
}
...
float Adafruit_INA219::getShuntVoltage() {
  int16_t value;
  value = getShuntVoltage_mV()/ 1000;
  return value   ;
}
/*!
