...
******************************************************************************/
//=====================================================================000==
//  INTERFACCIA MRAA - ARDUINO
//=====================================================================000==

#include "mraa.hpp"  ///#include <Wire.h>
#include <ArduinoDummy.h>
///#include "Arduino.h"  //funzione constatrain()
/*template<class T>
...
uint8_t constrain(const uint8_t& x, const T& a, const T& b){
    if(x < a) {
        return a;
    }
    else if(x > b) {
...
#include "sx1509.h"///#include "SparkFunSX1509.h"
...
{
	  /*_i2c->address(SX1509_ADDRESS);  
	  _i2c->writeByte(registerAddress);
	  _i2c->address(SX1509_ADDRESS);
	_i2c->read(rx_tx_buf, 1);
	byte value = rx_tx_buf[0];*/
	byte value =_i2c->readReg(registerAddress);
...
unsigned int SX1509::readWord(byte registerAddress)
{
  ...
  _i2c->writeByte(registerAddress);
  _i2c->address(SX1509_ADDRESS);
  _i2c->read(rx_tx_buf, 2);
 
  unsigned int value = ( (rx_tx_buf[0]) << 8) | rx_tx_buf[1];	
  return value;
...
void SX1509::readBytes(byte firstRegisterAddress, byte * destination, byte length)
{
	byte readValue;
  	_i2c->address(SX1509_ADDRESS); 
	_i2c->writeByte(firstRegisterAddress);
	_i2c->address(SX1509_ADDRESS);
	_i2c->read(rx_tx_buf, length);
	for (int i=0; i<length; i++)
	{
		destination[i] = rx_tx_buf[i] ;
	}
...
{
  	_i2c->address(SX1509_ADDRESS); 
   	_i2c->writeReg(registerAddress,writeValue);	
...
void SX1509::writeWord(byte registerAddress, unsigned int writeValue)
{
  	...
  	_i2c->address(SX1509_ADDRESS); 
   	_i2c->writeWordReg(registerAddress, writeValue);	
   
   //rx_tx_buf[0] = (uint8_t)(value & 0b0000000011111111);
   //rx_tx_buf[1] = (uint8_t)(value >> 8) & 0b0000000011111111;
   //_i2c->write(rx_tx_buf, 2);
